...
struct G_graph_ {
  int nodecount;
	G_nodeList mynodes; // 开头的node
  G_nodeList mylast;  // 结尾的node
};

//G_node节点定义
struct G_node_ {
  G_graph mygraph; // 这个node所在的图
  int mykey; // 第几个加入这个图的节点
  G_nodeList succs;
  G_nodeList preds;
...
void G_addEdge(G_node from, G_node to) {
  ...
  if (G_goesTo(from, to)) return;//已经存在边
...
void G_enter(G_table t, /*G_node node*/void *label, /*void *value*/void *node)
{
  TAB_enter(t, label, node);
}

void *G_look(G_table t, /*G_node node*/void *label)
{
